...
class CrossCoder(nn.Module):
    ...
    def get_losses(self, x):
        ...
        # explained_variance_A 求和形式
        per_token_l2_loss_A = (x_reconstruct[:, 0, :] - x[:, 0, :]).pow(2).sum(dim=-1).squeeze() #(batch,1,d)->(batch)
        total_variance_A = (x[:, 0, :] - x[:, 0, :].mean(0)).pow(2).sum(-1).squeeze() #(batch,1,d)->(batch) 
        explained_variance_A = 1 - per_token_l2_loss_A / total_variance_A

        # explained_variance_B 求和形式
        ...
        # l1 loss （batch,d_hidden)[模长] * (batch,hidden)[act]
